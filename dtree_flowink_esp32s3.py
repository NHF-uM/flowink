...
preserved_paths = [
    ...
    'zephyr/dts/bindings',
    'zephyr/dts/vendor/espressif',
    'zephyr/boards/espressif/esp32s3_devkitc',
]
...
# ====================== 新增配置项 ======================
# 额外需要在VSCode中显示的路径（支持相对路径片段匹配）
# 只要exclude列表中的路径包含这里的任何一个字符串，就会被从exclude中移除
# 例如：填写 'zephyr/drivers' 会让所有drivers目录下的文件都显示出来
# 填写 'gpio' 会让所有路径中包含gpio的文件和目录都显示出来
additional_include_paths = [
    'zephyr/dts/bindings/led_strip/worldsemi,ws2812-i2s'
]
# ======================================================
...
def collect_bindings_yaml_excludes(base_dir):
    """
    收集zephyr/dts/bindings下的所有yaml文件，排除包含"esp"的文件
    返回相对路径列表
    """
    bindings_path = os.path.join(base_dir, 'zephyr/dts/bindings')
    yaml_excludes = []
    
    if not os.path.exists(bindings_path):
        return yaml_excludes
    
    # 递归遍历bindings目录下的所有yaml文件
    for root, dirs, files in os.walk(bindings_path):
        for file in files:
            if file.endswith('.yaml') or file.endswith('.dts') or file.endswith('.dtsi'):
                # 检查是否包含"esp"
                if 'esp' not in file:
                    full_path = os.path.join(root, file)
                    rel_path = os.path.relpath(full_path, base_dir)
                    yaml_excludes.append(rel_path)
...
if __name__ == "__main__":
    ...
    # 添加bindings下除esp外的所有yaml文件到exclude列表
    yaml_excludes = collect_bindings_yaml_excludes(base_dir)
    edirs.extend(yaml_excludes)
    print(f'collected {len(yaml_excludes)} bindings yaml files to exclude')

    # ====================== 新增处理逻辑 ======================
    # 处理额外需要显示的路径，从exclude列表中移除
    if additional_include_paths:
        original_count = len(edirs)
        # 过滤掉所有包含additional_include_paths中任意字符串的路径
        edirs = [
            path for path in edirs
            if not any(include_path in path for include_path in additional_include_paths)
        ]
        removed_count = original_count - len(edirs)
        print(f'removed {removed_count} entries from exclude list based on additional_include_paths')
    # ======================================================
    ...
    defs = [x for x in defs if x.split('=')[0] not in discard_macros]
    defs = list(set(defs))
    defs.sort()

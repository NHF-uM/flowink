...
static void btn_click_work_handler(struct k_work *work);
static void btn_long_press_work_handler(struct k_work *work);
...
int main(void)
{
	...
	if (ret != 0)
	{
		LOG_ERR("failed to configure interrupt btn pin\n");
		return 0;
	}

	gpio_init_callback(&button_cb_data, button_pressed, BIT(btn_wakeup.pin));
	gpio_add_callback(btn_wakeup.port, &button_cb_data);
